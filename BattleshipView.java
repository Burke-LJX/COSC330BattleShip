...
public class BattleshipView extends Grid{
    ...
    public void createAndShowMainScreen() {
        ...
        String blankTileImgPath = "/images/defaultTile.png";
...
JPanel initializeButtonPanel(int windowWidth) {
    ...
    protected void placeShipRandomly(Ship ship, PlayerGrid playerGrid) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (isHorizontal) {
                playerGrid.getTile(row, col + i).occupyTile(ship.getShipType());
                playerGrid.getTile(row, col + i).getTileButton().setIcon(shipImgArray[i]);
            } else {
                playerGrid.getTile(row + i, col).occupyTile(ship.getShipType());
                playerGrid.getTile(row + i, col).getTileButton().setIcon(shipImgArray[i]);
            }
        }
